...
class DiffusionFitter(ParticleFinder):
    ...
    def hist_step_size(self, histCut=None, mode='Dist_abs', n=1, numBin=None):
        '''
    ...
    def velocity_measurements(self):
        ...
        # calculating the max displacement using Pythagorus (and adusting from
        # pixels to microns)
        ...
        # calculating number of frames at which particle achieves
        # max displacement
        df['frame_diff'] = df.groupby('particle')['frame'].apply(lambda x: x -
                                                                 x.iloc[0])

        # finding the final displacement
        final_d = df.groupby('particle')['disp'].apply(lambda x: x.iloc[-1])
        frames_at_final_d = (df.groupby('particle')['frame_diff'].
